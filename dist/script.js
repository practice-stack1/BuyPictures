...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  window.addEventListener('resize', function () {
    //! при раптовій зміні величини екрана ми ховаємо наше меню
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFuntion = function calcFuntion() {
    sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionBlock.value); //! виконуютья умови виведення інформації
  ...
  txtInputs.forEach(function (input) {
    input.addEventListener('keypress', function (e) {
      if (e.key.match(/[^а-яё 0-9]/ig)) {
        //! забороняє використовувати латиницю
        e.preventDefault();
      }
    });
  });
...
var drop = function drop() {
  ...
  var fileInputs = document.querySelectorAll('[name="upload"]'); //? Кожну подію ми присвоюємо усім інпутам
  ...
  function unhighlight(item) {
    item.closest('.file_upload').style.border = "none"; //! зміна стилів при скидувані файлу в залежності від батька даного поля
...
var filter = function filter() {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      //! кожен елемент контейенра зфото
  ...
  var typeFilter = function typeFilter(markType) {
    ...
    no.classList.remove('animated', 'fadeIn'); //! якщо ми передали селектор із певними типами, то відображаємо їх, якщо ніто виводимо блокк повідомлення
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target; //! якщо користувач клікнув на елемент меню, то конкретно йогму присврюємо клас ктивності
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); //? розбиваємо строку на масив із назви і типу файлу

      arr[0].length > 6 ? dots = "..." : dots = '.';
      var name = arr[0].substring(0, 6) + dots + arr[1]; //! створюємо півім'я + точки + тип файлу
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      statusMessage.classList.add('status'); //! добавляємо сповіщення-картинку в батьківський блок і приховуємо поточну форму

      item.parentNode.appendChild(statusMessage); //! форма зникає візуально і фізично із вікна

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); //? створюємо картинку-повідомлення і закидуємо в блок вище

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); //? закидуємо в той же блок текстове повідомлення також

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage);
      var formData = new FormData(item); //! визначаємо з яким модальним вікном ми співпацюємо
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        ...
        clearInputs();
        setTimeout(function () {
          statusMessage.remove(); //! візуалізовуємо форму після відправки на сервер
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    ...
    i = 0,
        ...
        val = this.value.replace(/\D/g, ''); //! при видалені елементів маски, ми їх відновлюємо
...
var modals = function modals() {
  ...
  function bindModals(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        //? завдяки такій конструкції на декілька одинакових селекторів ми зможемо повісити події
    modal = document.querySelector(modalSelector),
        close = document.querySelector(closeSelector),
        window = document.querySelectorAll('[data-modal]'),
        scroll = calcScroll(); //!  на кожен елемент ми вішамо опрацювання події

    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        if (e.target) {
          //! якщо клікнемо на посилання
          e.preventDefault();
        }

        if (destroy) {
          //! елемент, який провокує виклик модального вікна видаляється, якщо ми на нього клікаємо
          item.remove();
        }
        ...
        window.forEach(function (item) {
          //! метод для закриття усіх модальних вікон навипадок наявності двох послідовно відкриваємих модальних вікон
          item.style.display = 'none';
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); //! при відкритті вікна в нас буде відступ рівний зникаючому скролу
      });
    });
    ...
    modal.addEventListener('click', function (e) {
      if (e.target === modal) {
        //! клікаємо на підлошку для модального вікна для закриття і забороняємо це робити при виконанні певних операцій в
        window.forEach(function (item) {
          item.style.display = 'none';
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; //? визначаємо чи відкрите десь у нас уже модальне вікно, якщо так, то ми не виводимо дане

      document.querySelectorAll('[data-modal]').forEach(function (item) {
        if (getComputedStyle(item).display !== 'none') {
          //! перебираємо усі модальні вікна і визначаємо їх компютет стилі
          display = 'block';
        }
      });

      if (!display) {
        //! якщо в даний момент жодне вікно не показується
        document.querySelector(selector).style.display = "block";
        document.body.style.overflow = "hidden";
        var scroll = calcScroll(); //? даємо доступ нашій змінній scroll до методу
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //! визначення висоти сторніки для нових і старших браузерів
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector); //! проявляє елемент при скролі до певної відстані
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      //? шукаю всі посилання, які вміст атрибуту яких посинається з #
  speed = 0.3;
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var widthTop = document.documentElement.scrollTop,
          ...
          //? верхня границя елемента до якого ми будемо скролити
      start = null;
      ...
      function step(time) {
        ...
        if (r != widthTop + toBlock) {
          //! якщо ми долистали до потрібного пікселя то закінчуєо анімацію
          requestAnimationFrame(step);
        } else {
          location.hash = hash;
        }
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); //! візуалізація готових карточок
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      ...
      card.classList.add('col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1'); //! створюємо структуру картки
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  try {
    ...
    prevBtn.addEventListener('click', function () {
      plusSlides(-1); //? додаємо анімацію перелистування для попередньо виведеного слайду при його перегортуванні
  ...
  activateAnimation(); //! при наведені на слайдер відключаємо автопрокрутку слайде
